...
KOLLISION:
	...
	; Nach dem Zurücksetzen von PD3 ca 20 ms warten.


	; Interrupt Flag zurücksetzen. Ansonsten wird 
	; sonst sofort nach RETI wieder ein Interrupt ausgelößt
	ldi r16, (1<<INTF1)
	out GIFR, r16
...
; Initialisierung des 16-bit Stackpointer (SPH und SPL sind 0 nach dem Prozessorreset)
	...
	; bei Flankenwechsel nicht möglich. Daher nur den Idle Mode verwenden (SM0=0,SM1=0,SM2=0)
	; Sleep enable setzen
	LDI TEMP1, MCUCR
	ORI TEMP1, (1<<SE)
	OUT MCUCR, TEMP1
